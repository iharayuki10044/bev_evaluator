...
BEVEvaluator::BEVCalucurator(void)
:nh("~")
{
	nh.param("PKG_PATH", PKG_PATH, {"/home/amsl/ros_catkin_ws/src/bev_evaluator/bev_img"});

    bev_flow_estimator_subscriber = nh.subscribe("/cloud/dynamic", 10, &BEVEvaluator::pc_callback, this);
    bev_evaluator_subscriber = nh.subscribe("/odom", 10, &BEVEvaluator::odom_callback, this);
...
void BEVCalcurator::executor(void)
{
	...
	while(ros::ok()){
        ...
        bev_flow_evaluator_callback_flag = false;
